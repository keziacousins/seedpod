...
- **`reference-code/` is read-only and must NEVER be committed** — it contains `.env`, `admin-api-key.txt`, `db/`, `logs/`, and an embedded `.git` with dirty history. It is local archaeology only; it stays gitignored. Salvage means copying logic forward behind the new interfaces, citing the v1 path (`reference-code/seedpod/...`) in the PR/commit.
